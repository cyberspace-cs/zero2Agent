...
> 来源：腾讯 Agent 岗终面；[去哪儿 AI 全栈 AI 面](https://www.nowcoder.com/feed/main/detail/9cf516b3c2404100baeac52564e40709)
...
对多层 JSON 还要把“语法正确”和“契约正确”拆开：优先使用模型 API 的结构化输出能力约束 Schema；宿主收到结果后只解析一次，再递归校验必填键、类型、枚举、`additionalProperties`、数组元素和跨字段不变量。解析失败或嵌套层级不合法就返回结构化错误并有限重试，不能用正则补括号后直接进入业务链路。需要稳定键名时由 Schema 和版本化 DTO 决定，不能让模型根据自然语言临时发明字段。
...
> 来源：腾讯 Agent 岗终面【百度大模型研发二面追问：Agent 如何选择合适工具】【[深信服Agent开发实习生一面二面，长时间被吊着，最终被横向掉了](https://www.nowcoder.com/feed/main/detail/14b2c379ae434062a009aefea9fc5df9)追问：现在有100多个工具，AI想调用的时候能找到自己想要的接口吗？怎么保障？；工具检索是如何做的？】
...
| 传输方式 | 通信机制 | 适用场景 |
|---------|---------|---------|
| stdio | Client 启动 Server 子进程，经该进程的 stdin/stdout 交换 JSON-RPC；stderr 可承载日志 | 单 Host 管理的本地工具，生命周期和凭证继承边界清晰 |
| Streamable HTTP | 独立 Server 进程提供单一 MCP HTTP 端点，通过 POST/GET 通信，可选 SSE 传递多条服务端消息 | 远程、多客户端或需要独立部署和扩缩容的服务 |

[MCP 2025-11-25 Transport 规范](https://modelcontextprotocol.io/specification/2025-11-25/basic/transports)只定义 stdio 和 Streamable HTTP 两种标准传输；后者替代旧版 HTTP+SSE，SSE 是可选的流式承载方式，不应再把“HTTP/SSE”当成并列的新传输。stdio 的 Server 随 Client 以子进程启动，适合把文件和本地 CLI 权限限制在单一 Host；Streamable HTTP 的 Server 是可处理多个连接的独立服务，更适合放在网关、负载均衡和多副本之后。

远程化不只是换 URL：Streamable HTTP Server 必须校验 `Origin` 防 DNS rebinding，本地部署应只绑定 localhost，并对所有连接做适当认证。生产系统还要在网关完成 TLS、身份到租户/scope 的映射、限流和审计；有状态会话需要共享或粘性路由、会话过期和安全的 `MCP-Session-Id`，无状态实现才能任意横向扩容。断线不等于取消，请求重放、SSE 恢复和副作用幂等也必须分别设计。

**协议设计的核心原则**：
- **状态由实现显式声明**：Server 可以无状态，也可以使用协议会话；是否可重放和幂等仍由具体 Tool 契约决定
...
**差距在哪**：新手把 MCP 等同于调 API——完全忽略了三角色架构、初始化握手、能力协商这些协议层设计。高手能完整描述从初始化到工具发现到调用到结果回传的全链路，且清楚 stdio 和 Streamable HTTP（可选 SSE）两种标准传输的适用场景。面试官考的是你对 MCP 协议的理解是停留在“听说过”还是“真的看过协议规范”。
...
> 来源：遥望科技 Agent开发一面【[深信服Agent开发实习生一面二面，长时间被吊着，最终被横向掉了](https://www.nowcoder.com/feed/main/detail/14b2c379ae434062a009aefea9fc5df9)追问：动态工具注册中心是怎么做的？动态注入是怎么去注入的？；工具注册中心的构建过程中有什么难点吗？】
