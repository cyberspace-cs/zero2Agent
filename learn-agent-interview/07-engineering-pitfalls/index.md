...
**追问：Agent 从小流量试点变成全量开放后，成本预算和推理容量怎么治理？**

不能只把单任务 Token 上限乘以用户数。先按任务类型统计到达率、并发时长、模型调用次数、上下文长度和工具等待，形成“每任务成本 + 峰值容量”基线；再给租户、任务和执行阶段分配独立预算。意图识别、抽取和校验优先走小模型或确定性代码，复杂推理才升级大模型；队列接近容量时执行限流、延迟低优先级任务或降级到可解释的简化流程。

预算治理还要和质量一起看。按任务切片监控成功率、P95 延迟、单次成功任务成本、超预算率和人工接管率，避免为了省 Token 把任务做失败。扩量采用分阶段配额和容量压测，模型、Prompt 或工具变更后重新估算，而不是沿用试点阶段的平均值。

---

### Q：开发 Agent 时踩过什么坑？

> 来源：Agent 岗面试高频题 / [阿里国际 Agent 开发三面](https://www.nowcoder.com/feed/main/detail/747f07e71f4448bebdce6ada5de800cd) / [快手 AI 全栈一面](https://www.nowcoder.com/feed/main/detail/a30242712e8d456c839ff4223470f491)
...
“代码能运行”只是最低门槛，不代表它满足业务不变量、异常边界、性能预算和长期可维护性。质量门禁还要把项目规范变成可执行约束：格式、类型、lint、测试、安全扫描由 Hooks 或 CI 确定性执行，人工 Review 负责业务语义和架构影响；模型自评只能提供线索，不能替代这些门禁。
...
> 来源：字节 Agent 实习一面 【小红书数据库智能化二面追问：产品层面的等待体验优化】【小舒一面追问：异步调用与线程阻塞】 / [互联网金融 Agent 开发三面](https://www.nowcoder.com/feed/main/detail/88c55ee65af04ac98c218b9d17c47a71)【[9/4泰隆一面](https://www.nowcoder.com/feed/main/detail/738ff92bcd9049c5afc32d8f63226b79)追问：如何提高大模型响应速度？】
...
进一步说，Spec 不应只是一段更长 Prompt，而应是版本化、可验收的工程契约：定义目标、非目标、用户场景、接口/数据不变量、约束、验收用例和未决问题。[GitHub Spec Kit](https://github.com/github/spec-kit)把流程组织为 constitution、specify、plan、tasks 等阶段；这是一个具体工具实现，团队可以采用相同原则而不绑定它。

Coding Agent 执行时，每个任务和代码变更都应引用 `spec_id / requirement_id`，Verifier 从验收条款生成独立测试。发现实现与 Spec 冲突时，Agent 应分别提出 code diff 或 spec change，不能同时放宽验收标准并修改代码来制造“通过”。探索阶段可以做带预算的 Spike，但发现必须回写成待审 Spec diff，稳定需求再进入强门禁。
...
> 来源：蚂蚁集团智能体与大模型应用二面【[字节 AI Agent 开发岗一面](https://www.nowcoder.com/discuss/926273296180547584)追问：如何将 Spec-Driven Development 与 Agent 结合？】【[DeepSeek Harness 方向面试](https://www.nowcoder.com/discuss/925527442863714304)追问：Spec-driven development 在 AI 时代如何与 Agent 结合？】
...
> 来源：视频面经汇总 / [百度 Agent 二面](https://www.nowcoder.com/feed/main/detail/bca7dc14bd654e91b89792608111b211)
...
```mermaid
flowchart LR
    A["LangGraph\nstream_mode=updates"] -->|"async generator"| B["FastAPI\nSSE 包装"]
    B -->|"event stream"| C["前端\nEventSource"]
    C --> D["按 node_name\n更新 UI 组件"]
```
