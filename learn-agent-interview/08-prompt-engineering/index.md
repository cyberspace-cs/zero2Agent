...
> 来源：蚂蚁AI应用开发二面；[OPPO IT 开发一面](https://www.nowcoder.com/discuss/923561467092160512)【[要务科技-面筋](https://www.nowcoder.com/discuss/926539013991796736)追问：Skill的渐进式披露概念了解吗？】【[pdd agent二面](https://www.nowcoder.com/feed/main/detail/f5e7351df8364147ac8da085b99d9d18)追问：Agent怎么实现不同任务的隔离？】
...
> 来源：Agent开发八股合集（南京大学）；[钉钉一面](https://www.nowcoder.com/discuss/923765750446202880)
...
自动调优的通用闭环是“定义搜索空间 → 生成候选 → 在调优集上评测 → 选择候选 → 冻结集验收 → 灰度发布”，不要求一定使用 DSPy。以 DSPy 为例，[MIPROv2 官方文档](https://dspy.ai/api/optimizers/MIPROv2/)说明它可联合搜索 instruction 与 few-shot 示例，并通过候选生成和 Bayesian Optimization 选择组合；这只是搜索器，最终质量仍由 Metric 和数据决定。

工程上必须隔离训练/候选生成集、调参集和从未参与搜索的冻结验收集，避免 Optimizer 或 Judge 反复看到同一批 case 后过拟合。搜索空间限定可变模块、长度、示例数和禁止项；效果、延迟、Token、费用与安全按多目标门槛或 Pareto 前沿选择，不能只追一个 Judge 分数。评测固定模型、采样参数、依赖和缓存策略，Judge 先用人工样本校准；候选在无真实副作用的隔离环境中运行，通过回归后再 shadow/灰度，并保留旧 Prompt 的不可变版本和快速回滚指针。这些是通用发布约束，不是 MIPROv2 自动提供的保证。
...
> 来源：视频面经汇总；[快手 AI 全栈一面](https://www.nowcoder.com/feed/main/detail/a30242712e8d456c839ff4223470f491)
...
> 来源：[OPPO AI 全栈一面](https://www.nowcoder.com/discuss/920830730643443712)（2026-08-23）
