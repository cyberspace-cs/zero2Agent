...
> 来源：网易 AI Agent 开发实习；[阿里国际 Agent 开发三面](https://www.nowcoder.com/feed/main/detail/747f07e71f4448bebdce6ada5de800cd)；[字节中国交易与广告 AI 全栈二面](https://www.nowcoder.com/feed/main/detail/0f77410f8b1b4daca879d5ff99c7ae07)
...
混合方案要先写清责任矩阵：合规红线、资格校验、金额上限和不可逆操作由规则做硬约束；模型负责意图理解、语义判断和规则难以穷举的长尾。两者都输出结构化决策，携带规则/模型版本、置信度、证据和拒绝原因。发生冲突时，硬性 deny 不得被模型覆盖；模型低置信或证据不足时回退规则或转人工；确需模型覆盖的软规则必须有显式白名单、审计和业务 owner，而不是让最后返回的结果自然胜出。

上线前用历史流量回放比较“纯规则、纯模型、混合策略”，按关键风险、长尾覆盖、误拒、人工率、延迟和成本切片；上线时先 shadow，再按租户或场景灰度，并同时保留旧规则与模型版本以便回滚。模型学习线上数据前要过滤规则兜底和人工修改造成的选择偏差，避免把临时策略重新学成永久事实。
...
> 来源：字节 TikTok AI Agent 开发一面（2026-08-13）；[字节中国交易与广告 AI 全栈二面](https://www.nowcoder.com/feed/main/detail/0f77410f8b1b4daca879d5ff99c7ae07)
...
转写后复核不能只让同一个模型“再看一遍”。先用词表、时间戳连续性、说话人一致性和实体格式等确定性规则发现异常，再让 Reviewer 接收音频片段引用、原转写、候选修订和上下文，输出固定 Schema：`span、error_type、proposal、evidence、confidence`。Reviewer 与生成器同源时要警惕相关错误，困难样本可换独立模型或交人工；自动修订保留原文和证据 span，高风险实体、低置信冲突与规则/模型分歧进入人工抽检。评测同时记录首版、复核后和人工终版的差异，确认复核确实提升端到端摘要与待办质量，而不是只让文本更流畅。
...
> 来源：[百川智能医疗大模型后训练一面](https://www.nowcoder.com/feed/main/detail/d209892dc2cf43c6859a776c5e526eca)（2026-08-22）【[阶跃星辰（Stepfun）- 大模型算法岗（Post-train）](https://www.nowcoder.com/discuss/926273007276814336)追问：医疗 Skill 的设计动机及与 HIS 协同方案？】
...
> 来源：百度大模型研发工程师二面（2026-08-21）【[百度 - Agent 研发岗（架构方向）](https://www.nowcoder.com/discuss/926273622006665216)追问：何为 Agent 应用？C 端 Agent 的架构模块？】
